    ...
    <div className="flex flex-col items-center justify-center min-h-[75vh] w-full text-white px-4">
      ...
      <div className="text-center max-w-2xl w-full mb-10">
        ...
        <div className="w-full h-1.5 bg-[#1e293b] rounded-full mt-10 overflow-hidden">
          <div className="h-full bg-blue-500 rounded-full w-2/3 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
        </div>
      </div>
      ...
      <div className="w-full max-w-2xl space-y-4">
        ...
        {/* GitHub */}
        <div className="bg-[#121a2f] rounded-2xl border border-[#1e293b] shadow-sm overflow-hidden">
          <div className="p-5 md:p-6 flex items-center justify-between">
            ...
            {githubConnected ? (
              <span className="flex items-center gap-1.5 text-emerald-400 text-sm font-bold bg-emerald-900/10 px-3 py-1.5 rounded-lg border border-emerald-900/30 whitespace-nowrap">
              ...
              <button
                ...
                onClick={() => { setShowGithubInput(v => !v); setShowLinkedinInput(false); }}
                className="bg-[#1e293b] hover:bg-[#334155] border border-[#334155] px-5 py-2 rounded-xl text-sm font-bold transition whitespace-nowrap"
              >
                Connect
              </button>
            )}
          </div>
          {showGithubInput && !githubConnected && (
            <div className="px-5 pb-5 flex gap-3 items-center border-t border-[#1e293b] pt-4">
              <input
                id="github-url-input"
                type="url"
                value={githubUrl}
                ...
                className="flex-1 bg-[#0b1120] border border-[#1e293b] rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition"
              />
              <button
                id="github-save-btn"
                onClick={handleSaveGithub}
                disabled={saving || !githubUrl.trim()}
                className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 px-5 py-2.5 rounded-xl text-sm font-bold transition whitespace-nowrap"
              >
                Save
              </button>
              <button onClick={() => setShowGithubInput(false)} className="text-slate-400 hover:text-white text-sm px-2 transition">✕</button>
            </div>
          )}
        </div>
        ...
        <div className="bg-[#121a2f] rounded-2xl border border-[#1e293b] shadow-sm overflow-hidden">
          <div className="p-5 md:p-6 flex items-center justify-between">
            ...
            {linkedinConnected ? (
              ...
              <button
                ...
                onClick={() => { setShowLinkedinInput(v => !v); setShowGithubInput(false); }}
                className="bg-[#1e293b] hover:bg-[#334155] border border-[#334155] px-5 py-2 rounded-xl text-sm font-bold transition whitespace-nowrap"
              >
                Connect
              </button>
          ...
          {showLinkedinInput && !linkedinConnected && (
            <div className="px-5 pb-5 flex gap-3 items-center border-t border-[#1e293b] pt-4">
              <input
              ...
              <button onClick={() => setShowLinkedinInput(false)} className="text-slate-400 hover:text-white text-sm px-2 transition">✕</button>
            </div>
          )}
        </div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">
                {resumeUploaded ? `Uploaded: ${resumeFile?.name}` : 'Upload for AI analysis & auto-fill'}
              </p>
            </div>
          </div>
